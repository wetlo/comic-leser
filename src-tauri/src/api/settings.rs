...
// TODO: this whole section might need a rewrite
#[tauri::command]
...

    {
        ...
        match (
            ...
            (Some(sel), Some(l)) if sel == id => {
                ...
                select_library(id, settings.clone(), library, app).await?
            }
            (Some(_), None) => {
                let mut lib = library.access_option().await;
                *lib = None;
...
async fn persist(settings: tauri::State<'_, SettingsState>) -> Result<(), String> {
    settings.access().await?.persist().await.str_err()
}
